...
class RavencoreAssetRightclickPopupMenu(object):
    ...
    def __init__(self, parentBinding, parentFrame, assetData={}):
        ...
        if assetData != None:
            if assetData.__contains__('has_ipfs'):
                self._hasIpfs =  assetData['has_ipfs']
        else:
            self._dataNone = True
    ...
    def __createMenuItems__(self):
        ...
        #
        nid = wx.NewId()
        _addBk = self._menu.Append(nid, "Add in Bookmarks" )
        _addBk.SetBitmap(self.parent_frame.RessourcesProvider.GetImage('addbkmrk_co'))
        self.parentBinding.Bind(wx.EVT_MENU, self.addInBkmrk, id=nid)
        ...
        #
        nid = wx.NewId()
        _Navigate = self._menu.Append(nid, "Navigate" )
        _Navigate.SetBitmap(self.parent_frame.RessourcesProvider.GetImage('hierarchicalLayout'))
        self.parentBinding.Bind(wx.EVT_MENU, self.navigateAsset, id=nid)
